 ...
 <template>
	<!--<div>
		<div style="padding-top:20px;">
			<template v-if="this.screen_id=='1'">
				<v-row>
					<v-col xl="1" md="1" sm="1" cols="1" align-self="center" style="text-align:end;padding-top:0px; padding-bottom:0px;padding-right:0px;">
						<v-btn width="100%" color="primary" style="color:white;font-weight:bold;" class="fontDefault" @click="ChangeSrceen('1')">URL</v-btn>
					</v-col>
					<v-col xl="1" md="1" sm="1" cols="1" align-self="center" style="text-align:end;padding-top:0px; padding-bottom:0px;padding-right:0px;">
						<v-btn width="100%" color="white" style="color:black;font-weight:bold;" class="fontDefault" @click="ChangeSrceen('2')">계정</v-btn>
					</v-col>
					<v-col xl="1" md="1" sm="1" cols="1" align-self="center" style="text-align:end;padding-top:0px; padding-bottom:0px;padding-right:0px;">
						<v-btn width="100%" color="white" style="color:black;font-weight:bold;" class="fontDefault" @click="ChangeSrceen('3')">에러코드</v-btn>
					</v-col>
					<v-col xl="6" md="6" sm="6" cols="6" align-self="center" style="text-align:end;padding-top:0px; padding-bottom:0px;padding-right:0px;">
					</v-col>
				</v-row>
			</template>
			<template v-if="this.screen_id=='2'">
				<v-row>
					<v-col xl="1" md="1" sm="1" cols="1" align-self="center" style="text-align:end;padding-top:0px; padding-bottom:0px;padding-right:0px;">
						<v-btn width="100%" color="white" style="color:black;font-weight:bold;" class="fontDefault" @click="ChangeSrceen('1')">URL</v-btn>
					</v-col>
					<v-col xl="1" md="1" sm="1" cols="1" align-self="center" style="text-align:end;padding-top:0px; padding-bottom:0px;padding-right:0px;">
						<v-btn width="100%" color="primary" style="color:white;font-weight:bold;" class="fontDefault" @click="ChangeSrceen('2')">계정</v-btn>
					</v-col>
					<v-col xl="1" md="1" sm="1" cols="1" align-self="center" style="text-align:end;padding-top:0px; padding-bottom:0px;padding-right:0px;">
						<v-btn width="100%" color="white" style="color:black;font-weight:bold;" class="fontDefault" @click="ChangeSrceen('3')">에러코드</v-btn>
					</v-col>
			...
			<template v-if="this.screen_id=='3'">
				<v-row>
					<v-col xl="1" md="1" sm="1" cols="1" align-self="center" style="text-align:end;padding-top:0px; padding-bottom:0px;padding-right:0px;">
						<v-btn width="100%" color="white" style="color:black;font-weight:bold;" class="fontDefault" @click="ChangeSrceen('1')">URL</v-btn>
					</v-col>
					...
					<v-col xl="1" md="1" sm="1" cols="1" align-self="center" style="text-align:end;padding-top:0px; padding-bottom:0px;padding-right:0px;">
						<v-btn width="100%" color="primary" style="color:white;font-weight:bold;" class="fontDefault" @click="ChangeSrceen('3')">에러코드</v-btn>
					</v-col>
					<v-col xl="6" md="6" sm="6" cols="6" align-self="center" style="text-align:end;padding-top:0px; padding-bottom:0px;padding-right:0px;">
					</v-col>
				</v-row>
		...
		<template v-if="this.screen_id=='1'">
			<v-row style="margin:0 0 0 0; padding: 0 0 0 0 !important;" justify="center">
				<v-col xl="2" md="2" sm="2" cols="2" align-self="center">
					<v-select 
						...
						@change="getList"
						class="testselect"
						style="overflow-x: hidden; padding: 3px; margin-top:15px; font-size:13px;" > 
					</v-select>
				</v-col>
			...
			<v-row style="margin:0 0 0 0; padding: 0 0 0 0 !important;" justify="center">
				<v-col xl="12" md="12" sm="12" cols="12" align-self="center" style="width:100%; padding-bottom:0px;margin-bottom:0px;cursor:pointer;padding-top:0px; margin-top:0px;">
					<v-data-table no-data-text="조회된 URL이 없습니다."  disable-sort style="margin-bottom:0px;" @click:row="onClickRow" mobile-breakpoint="0" :headers="headers" :items="document" :loading="loading" :footer-props="{ 'items-per-page-options': [10, -1] }" :items-per-page="10">
						<template v-slot:item="{item, index}">
							<tr @click="onClickRow(item,index)">
								<template v-if="item.clm_change_yn=='N'">
									...
									<td>
										<v-select 
											...
											open-on-clear 
											persistent-hint
											@click="changeDocuments(item,index)"
								...
								<td> <span><v-icon style="overflow-x: hidden;overflow-y: hidden;" @click="fnc_icon_click('P', index)">mdi-plus</v-icon></span> </td>
		...
		<template v-else-if="this.screen_id=='2'">
			<v-row style="margin:0 0 0 0; padding: 0 0 0 0 !important;" justify="center">
				...
				<v-col xl="1" md="1" sm="1" cols="1" align-self="center">
					<v-btn width="100%" color="green" v-on:click="addList">등록</v-btn>
				</v-col>
			</v-row>
			<v-row>
				<v-col xl="3" md="3" sm="3" cols="3" v-for="(accountItem, index) in accountItems" :key="index">
					<div  style="width:100%; border: 2px solid; border-bottom: none; background-color: skyblue; display: flex;">
						<div style="width: 70%;" >
							<input style="align-self: center; width: 100%;" maxlength="10" v-model="accountItem.clm_title" />
						</div> 
						<div v-if="accountItem.clm_content_seq == '1'" style="float: right; text-align: end">
							...
							&nbsp;
							<i class="mdi mdi-pin" style="color: green;"  v-on:click="checkOrderSeq(accountItem, index)"></i>
							&nbsp;
							<i class="mdi mdi-content-copy" style="color: black;"  v-on:click="copyText(accountItem, index)"></i>
						</div>
						<div v-else-if="accountItem.clm_content_seq =='0'" style="float: right; text-align: end">
							<i class="mdi mdi-content-save" style="color: springgreen;" v-on:click="SaveContent(accountItem, index)"></i>
							&nbsp;
							<i class="mdi mdi-delete" style="color: red;" v-on:click="DeleteContent(accountItem, index)"></i>
							&nbsp;
							<i class="mdi mdi-pin-off" style="color: green;"  v-on:click="checkOrderSeq(accountItem, index)"></i>
							&nbsp;
							<i class="mdi mdi-content-copy" style="color: black;"  v-on:click="copyText(accountItem, index)"></i>
						</div>	
					</div>
					<textarea v-model="accountItem.clm_content" rows="10" style="border: 2px solid; resize: none; width:100% "></textarea>
				</v-col>
			</v-row>
		</template>
		<template v-else-if="this.screen_id=='3'">
			<v-row>
				<v-col xl="12" md="12" sm="12" cols="12" align-self="center" style="padding-bottom:0px;margin-bottom:0px;">
					...
					<v-row class="justify-end">
						<v-col xl="1" md="2" sm="2" cols="3" align-self="center"  style="padding-top:0px;margin-top:0px;">
						...
						<v-col xl="1" md="2" sm="2" cols="6" align-self="center"  style="padding-top:0px;margin-top:0px;">
					...
					<v-row>
						<v-col xl="6" md="6" sm="6" cols="6">
							<v-data-table  class="elevation-1" :footer-props="{ 'items-per-page-text': '페이지 당 행',  pageText: '총 {2} 항목 중 {0}-{1}'}" 
							 @click:row="ka_onClickRow" @dblclick:row="ka_ondbClickRow" :headers="ka_headers" :items="ka_document" @pagination="updatePage" :itemsPerPage="list_page" :loading="loading" :item-class="select_code_color" >
								<template slot="items" slot-scope="props">
									<td>{{ props.item.num }}</td>
									<td>{{ props.item.clm_code_id }}</td>
									<td>{{ props.item.clm_code_name }}</td>
									<td>{{ props.item.clm_use_yn }}</td>
									<td>{{ props.item.clm_comment }}</td>
								</template>
							</v-data-table>
						</v-col>
						<v-col xl="6" md="6" sm="6" cols="6">
		...
		<template v-else-if="this.screen_id=='9'">
			<v-row justify="center">							
				<v-col xl="12" md="12" sm="12" cols="12"  align-self="center" style="text-align:center; padding-bottom:0px;margin-bottom:0px;padding-top:300px;">
					<span style="font-size:50px;"> 로딩중입니다. 잠시만 기다려 주세요. </span>
				</v-col>
			</v-row>
		</template>
		<v-row justify="center">
				<v-dialog v-model="ErrorRegmodal" persistent max-width="850">
					...
					<v-card-text style="padding-bottom:0px;">
						<v-row >
							<v-col xl="12" md="12" sm="12" cols="12"  align-self="center"  style="padding-bottom:0px;margin-bottom:0px;padding-top:25px;">
								<span style="color:red; float: left; padding-top:20px;">*</span><v-text-field label="에러 코드명" v-model="clm_m_error_code_name" />
							</v-col>
						</v-row>
						<v-row >
							<v-col xl="12" md="12" sm="12" cols="12"  align-self="center"  style="padding-bottom:0px;margin-bottom:0px;padding-top:0px;">
								<span style="color:red; float: left; padding-top:20px;">*</span><v-text-field label="에러 코드 설명" v-model="clm_m_error_code_text" />
							</v-col>
						</v-row>
						<v-row>
							<v-col xl="4" md="4" sm="4" cols="4"  align-self="center"  style="padding-bottom:0px;margin-bottom:0px;padding-top:0px;">
								<v-text-field label="DB Host" v-model="clm_m_error_code_url" />
							</v-col>
							<v-col xl="4" md="4" sm="4" cols="4"  align-self="center"  style="padding-bottom:0px;margin-bottom:0px;padding-top:0px;">
								<v-text-field label="DB명" v-model="clm_m_error_code_dbName" />
							</v-col>
							<v-col xl="4" md="4" sm="4" cols="4"  align-self="center"  style="padding-bottom:0px;margin-bottom:0px;padding-top:0px;">
								<v-text-field label="PORT" v-model="clm_m_error_code_port" />
							</v-col>
						</v-row>
						<v-row>
							<v-col xl="6" md="6" sm="6" cols="6"  align-self="center"  style="padding-bottom:0px;margin-bottom:0px;padding-top:0px;">
								<v-text-field label="DB User" v-model="clm_m_error_code_dbId" />
							</v-col>
							<v-col xl="6" md="6" sm="6" cols="6"  align-self="center"  style="padding-bottom:0px;margin-bottom:0px;padding-top:0px;">
								<v-text-field label="DB PW" v-model="clm_m_error_code_dbPw" />
							</v-col>
						</v-row>
						<v-row>
							<v-col xl="12" md="12" sm="12" cols="12"  align-self="center"  style="padding-bottom:0px;margin-bottom:0px;padding-top:0px;">
								<v-text-field label="쿼리체크" v-model="clm_m_error_code_query_check" />
							</v-col>
							<v-col xl="12" md="12" sm="12" cols="12"  align-self="center"  style="padding-bottom:0px;margin-bottom:0px;padding-top:0px;">
								<v-text-field label="웹사이트체크" v-model="clm_m_error_code_web_check" />
							</v-col>
							<v-col xl="12" md="12" sm="12" cols="12"  align-self="center"  style="padding-bottom:0px;margin-bottom:0px;padding-top:0px;">
								<v-text-field label="프로그램체크" v-model="clm_m_error_code_program_check" />
							</v-col>
						</v-row>
					</v-card-text>
			...
			<v-row justify="center">
				<v-dialog  v-model="PhoneModal" persistent max-width="850">
					...
					<v-card-text style="padding-bottom:0px;">
						<v-row >
							<v-col xl="4" md="4" sm="4" cols="4" align-self="center" style="padding-bottom:0px;margin-bottom:0px;padding-top:32px;" >
								<span style="color:red; float: left; padding-top:20px;">*</span><v-text-field label="에러코드이름" v-model="clm_m_error_name" @click="errorModalOpen" />
							</v-col>
							<v-col xl="4" md="4" sm="4" cols="4" align-self="center" style="padding-bottom:0px;margin-bottom:0px;padding-top:32px;" >
								<span style="color:red; float: left; padding-top:20px;">*</span><v-text-field label="이름" v-model="clm_m_user_name" />
							</v-col>
							<v-col xl="4" md="4" sm="4" cols="4" align-self="center" style="padding-bottom:0px;margin-bottom:0px;padding-top:32px;" >
								<span style="color:red; float: left; padding-top:20px;">*</span><v-text-field label="연락처" v-model="clm_m_phone_number" />
							</v-col>
						</v-row>
					</v-card-text>
					<v-card-actions class="justify-end">
						<template v-if="this.$router.app.$route.query.update_yn=='Y'">
							...
							<template v-else-if="this.clm_kep_id !='' ">
		...
		<v-dialog v-model="dialog" persistent max-width="290">
			<v-card>
				...
				<v-card-text style="padding-bottom:0px;">
					<v-text-field label="ID" icon="person" v-model="username"/>
					<v-text-field label="Password" icon="lock" type="password" v-model="password" v-on:keyup.enter="LoginConfirm" />
				</v-card-text>
				<v-card-actions style="padding-top:0px;">
				<v-spacer></v-spacer>
				<v-btn color="black darken-1" text v-on:click="LoginConfirm">
					로그인
				</v-btn>
				</v-card-actions>
			</v-card>
...
<template style="height: 100%;">
	<v-layout style="width:100%;height: 100%;">
		<div class="menu_div" >
			...
			<!-- <template v-if="this.screen_authority_001001=='Y'">
